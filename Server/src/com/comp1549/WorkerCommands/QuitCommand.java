...
public class QuitCommand {
    ...
    public static void run(ServerWorker worker) throws IOException {
        ...
        System.out.printf((Messages.ClosingClientConnection) + "%n", worker.IP, worker.Port);
        worker.handleClientClose(); // Closes the client socket
        System.out.printf((Messages.ClosedClientConnection) + "%n", worker.IP, worker.Port);
        ServerMain.broadcastMessage(String.format(Messages.ConfirmedClientDisconnection, worker.formattedUsername)); // Broadcasts the user/client has disconnected.
