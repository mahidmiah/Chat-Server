...
public class QuitCommand {
    ...
    public static void run(ServerWorker worker) throws IOException {
        ...
        System.out.printf((Messages.Message_27) + "%n", worker.IP, worker.Port);
        worker.handleClientClose(); // Closes the client socket
        System.out.printf((Messages.Message_28) + "%n", worker.IP, worker.Port);
        ServerMain.broadcastMessage(String.format(Messages.Message_29, worker.formattedUsername)); // Broadcasts the user/client has disconnected.
